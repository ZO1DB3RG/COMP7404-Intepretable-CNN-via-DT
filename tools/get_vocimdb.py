...
#根据一个存储所有对应类别bounding box的.mat文件读取该类正确信息并用字典与图像一一对应
def readAnnotation(root_path, dataset, dataset_path, categoryName):
    ...
    dataset_data_namebatch_img_path = os.path.join(os.path.join(root_path, 'datasets'), "VOCdevkit","VOC2010","JPEGImages")
    mat = h5py.File(dataset_truth_file, 'r')
    truth = mat['truth']["obj"]
    objset = []
    for i in range(len(truth)):
        xmin = int(mat[(truth[i][0])]['bndbox']['Wmin'][()])
        ymin = int(mat[(truth[i][0])]['bndbox']['Hmin'][()])
        xmax = int(mat[(truth[i][0])]['bndbox']['Wmax'][()])
        ymax = int(mat[(truth[i][0])]['bndbox']['Hmax'][()])
