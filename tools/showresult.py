...
from tools.computeStability import computeStability, get_plot, save_g_x
...
def step_plot(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, layerID, epochnum, partList, partRate, imdb_mean):
    selectPatternRatio = 1.0
    patchNumPerPattern = 100
    if(label_name=='n01443537'):
        partList=[1]
    convnet = getConvNetPara(model)
    stability = get_plot(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, convnet, layerID, epochnum, partList, partRate, imdb_mean, selectPatternRatio, patchNumPerPattern)
    return stability

def step_grad(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, layerID, epochnum, partList, partRate, imdb_mean):
    selectPatternRatio = 1.0
    patchNumPerPattern = 100
    if(label_name=='n01443537'):
        partList=[1]
    convnet = getConvNetPara(model)
    stability = save_g_x(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, convnet, layerID, epochnum, partList, partRate, imdb_mean, selectPatternRatio, patchNumPerPattern)
    return stability
...
def getresult_plot(root_path,dataset,taskid_path, imdb_path, dataset_path, truthpart_path, label_name, model, layerID, epochnum, partList):
    partRate = 1
    imdb_mean_path = os.path.join(imdb_path, 'mean.mat')
    imdb_mean = load_imdb(imdb_mean_path,'mean')
    sta = []
    ...
    net_path = os.path.join(taskid_path,"net-" + str(epochnum) +".pkl")
    net = load_model(net_path)
    stability = step_plot(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, layerID, epochnum, partList, partRate, imdb_mean)
    return stability

def getresult_grad(root_path,dataset,taskid_path, imdb_path, dataset_path, truthpart_path, label_name, model, layerID, epochnum, partList):
    partRate = 1
    imdb_mean_path = os.path.join(imdb_path, 'mean.mat')
    imdb_mean = load_imdb(imdb_mean_path,'mean')
    sta = []
    ...
    net_path = os.path.join(taskid_path,"net-" + str(epochnum) +".pkl")
    net = load_model(net_path)
    stability = step_grad(root_path,dataset,dataset_path, truthpart_path, label_name, net, model, layerID, epochnum, partList, partRate, imdb_mean)
    return stability
...
def show_plot(epoch_num,taskid_path, imdb_path, root_path, args):
    if args.model in ['alexnet','vgg_m','vgg_s']:
        layerID = 6
    elif args.model in ['vgg_vd_16']:
        layerID = 14
    ...
    if args.task_name == 'classification_multi':
        ...
        stability = getresult_plot(root_path,args.dataset,taskid_path, imdb_path, dataset_path, truthpart_path, args.label_name, args.model, layerID, epoch_num, partList)
    print(stability)
    return stability

def show_grad(epoch_num,taskid_path, imdb_path, root_path, args):
    if args.model in ['alexnet','vgg_m','vgg_s']:
        layerID = 6
    elif args.model in ['vgg_vd_16']:
        layerID = 14
    ...
    truthpart_path = os.path.join(root_path, "data_input", args.dataset)
    stability = getresult_grad(root_path,args.dataset,taskid_path, imdb_path, dataset_path, truthpart_path, args.label_name, args.model, layerID, epoch_num, partList)
    return stability
